...
class MovieRecommender:
    ...
    def _rebuild(self):
        ...
        coords = pca.fit_transform(genre_matrix.toarray())
        self.x = coords[:, 0].tolist()
        self.y = coords[:, 1].tolist()
        self.z = coords[:, 2].tolist()
        self.primary_genre = [
            g.split()[0] if g else "Unknown" for g in self.df["genres"]
        ]
